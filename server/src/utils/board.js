...
export function validateBoard(board, size) {
  ...
  for (const row of board) {
    ...
    for (const value of row) {
      if (!Number.isInteger(value) || value < 1) return { valid:false, message:"Board values must be positive integers" };
...
export function markNumber(board, number) {
  return board.map(row => row.map(cell =>
    cell && cell.value === number ? { ...cell, marked:true } : cell
  ));
}
...
export function countCompletedLines(board) {
  ...
  for (let r=0;r<size;r++) {
    if (board[r].every(c => c.marked)) lines.push({type:"row", index:r});
  }
  for (let c=0;c<size;c++) {
    let ok = true;
    for (let r=0;r<size;r++) if (!board[r][c].marked) ok=false;
    if (ok) lines.push({type:"column", index:c});
  }
  let d1=true, d2=true;
  for (let i=0;i<size;i++) {
    if (!board[i][i].marked) d1=false;
    if (!board[i][size-1-i].marked) d2=false;
  }
  if (d1) lines.push({type:"diagonal", index:"main"});
  if (d2) lines.push({type:"diagonal", index:"anti"});
  return lines;
}
