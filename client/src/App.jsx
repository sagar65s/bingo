...
function Dashboard({ user, onLogout }) {
  ...
  const submitBoard = () => { if (!game || !board || !socket) return; const size = game.boardSize; if (!Array.isArray(board) || board.length !== size || board.some(row => !Array.isArray(row) || row.length !== size)) { setMessage("Please fill the complete board before submitting."); return } const values = board.flat().map(v => Number(v)); const expected = size * size; if (values.some(v => !Number.isInteger(v) || v < 1 || v > expected)) { setMessage(`Use numbers from 1 to ${expected}.`); return } if (new Set(values).size !== values.length) { setMessage("Duplicate numbers are not allowed on your board. Each number must be used once."); return } socket.emit("game:boardSubmit", { gameId: game.gameId, board: board.map(row => row.map(v => Number(v))) }); setMessage("Board submitted. Waiting for the other players...") };
  if (room && game) return <Game user={user} room={room} game={game} board={board} setBoard={setBoard} submitBoard={submitBoard} onLogout={onLogout} />;
