...
function Dashboard({user,onLogout}) {
  ...
  const [onlinePlayers,setOnlinePlayers]=useState([]),[invites,setInvites]=useState([]),[showCreatePassword,setShowCreatePassword]=useState(false),[showJoinPassword,setShowJoinPassword]=useState(false);

  useEffect(()=>{
    const s=createSocket(); setSocket(s); window.__bingoSocket=s;
    s.on("error:message",e=>setMessage(e.message));
    s.on("presence:list",e=>setOnlinePlayers(e.players||[]));
    s.on("presence:update",e=>setOnlinePlayers(e.players||[]));
    ...
    s.on("room:invite:response",e=>setMessage(e.message));
  ...
  const invitePlayer=targetUserId=>{if(!room||!socket)return;socket.emit("room:invite",{roomId:room.roomId,targetUserId})};
...
function Game({user,room,game,board,setBoard,submitBoard,onLogout,onBack,onExit,onRematch}){
  ...
  if(e.key==="ArrowRight") next=col<size-1 ? index+1 : index;
  else if(e.key==="ArrowLeft") next=col>0 ? index-1 : index;
  else if(e.key==="ArrowDown") next=row<size-1 ? index+size : index;
  else if(e.key==="ArrowUp") next=row>0 ? index-size : index;
  else if(e.key==="Enter") next=Math.min(size*size-1,index+1);
  else if(e.key==="Backspace" && !localBoard[row][col]) next=Math.max(0,index-1);

  if(next!==null){
    e.preventDefault();
    setActiveCell(next);
    requestAnimationFrame(()=>{
      const el=document.querySelector(`[data-cell="${next}"]`);
      el?.focus();
      el?.select?.();
    });
  }
