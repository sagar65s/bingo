...
function GameApp({user,logout}){const sock=useRef(),activeRoomId=useRef(null),[room,setRoom]=useState(null),[game,setGame]=useState(null),[online,setOnline]=useState([]),[invites,setInvites]=useState([]),[sent,setSent]=useState({}),[board,setBoard]=useState(null),[notice,setNotice]=useState(''),[leader,setLeader]=useState([]);const[msgRoom,setMsgRoom]=useState('Friends Room'),[pw,setPw]=useState(''),[showPw,setShowPw]=useState(false),[players,setPlayers]=useState(2),[joinId,setJoinId]=useState(''),[joinPw,setJoinPw]=useState(''),[showJoin,setShowJoin]=useState(false);
 ...
 useEffect(()=>{const s=createSocket();sock.current=s;window.__bingoSocket=s;const sync=e=>setOnline(e.players||[]);s.on('presence:list',sync);s.on('presence:update',sync);s.on('error:message',e=>setNotice(e.message));s.on('room:update',e=>{
  ...
  setRoom(e.room);setGame(null);setBoard(null);setInvites(v=>v.filter(x=>x.inviteId!==e.inviteId));setNotice('Joined room successfully')});s.on('room:invite',i=>setInvites(v=>v.some(x=>x.inviteId===i.inviteId)?v:[i,...v]));s.on('room:invite:response',e=>{setNotice(e.message);if(e.targetUserId)setSent(v=>{const n={...v};delete n[String(e.targetUserId)];return n})});s.on('room:started',e=>{
...
function Header({user,logout}){return <header className="top"><div className="logo"><i>B</i><b>BINGO</b></div><div className="user"><span>{user.username?.[0]?.toUpperCase()}</span><b>{user.username}</b><button onClick={logout}>Logout</button></div></header>}
...
function Play({user,game,room,board,setBoard,submit,leave,rematch,notice}){const size=game.boardSize,me=game.players.find(p=>String(p.userId)===String(user._id)),setup=game.status==='BOARD_SETUP',mine=board||Array.from({length:size},()=>Array(size).fill(''));const random=()=>{const a=Array.from({length:size*size},(_,i)=>i+1);for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]]}setBoard(Array.from({length:size},(_,r)=>a.slice(r*size,r*size+size).map(String)))};const edit=(idx,val)=>{const v=val.replace(/\D/g,'').slice(0,String(size*size).length);const flat=mine.flat();flat[idx]=v;setBoard(Array.from({length:size},(_,r)=>flat.slice(r*size,r*size+size)));};const key=(e,i)=>{let next=null,r=Math.floor(i/size),c=i%size;if(e.key==='ArrowRight')next=c<size-1?i+1:i;else if(e.key==='ArrowLeft')next=c?i-1:i;else if(e.key==='ArrowDown')next=r<size-1?i+size:i;else if(e.key==='ArrowUp')next=r?i-size:i;else if(e.key==='Enter')next=Math.min(size*size-1,i+1);if(next!==null){e.preventDefault();document.querySelector(`[data-cell="${next}"]`)?.focus()}};const called=new Set(game.calledNumbers||[]);
